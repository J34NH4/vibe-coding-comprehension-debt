...
class Solution:
    """Solution for 3Sum problem using two-pointer technique."""
    ...
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        sorted_nums = sorted(nums)  # Sort for two-pointer approach
        result_triplets = []
        ...
        for first_index in range(len(sorted_nums) - 2):
            ...
            remaining_triplets = self._find_two_sum_pairs(
                sorted_nums, first_index, self.TARGET_SUM - sorted_nums[first_index]
            )
    ...
    def _find_two_sum_pairs(self, sorted_nums: List[int], first_index: int, target_sum: int) -> List[List[int]]:
        """
        Find all pairs in remaining array that sum to target_sum.
        ...
        Returns:
            List of triplets where first element is at first_index
        """
        triplets = []
        ...
        while left_pointer < right_pointer:
            ...
            if current_sum == target_sum:
                # Found valid triplet
                triplet = [sorted_nums[first_index], sorted_nums[left_pointer], sorted_nums[right_pointer]]
