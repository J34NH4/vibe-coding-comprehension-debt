...
class Solution:
    ...
    def _calculate_paths_dynamic_programming(self, rows: int, columns: int) -> int:
        ...
        """
        # Create DP table where dp[i][j] represents paths to cell (i,j)
        dynamic_programming_table = [[0 for _ in range(columns)] for _ in range(rows)]
        
        # Initialize first row - only one way to reach any cell in first row
        for column_index in range(columns):
            dynamic_programming_table[0][column_index] = 1
            
        # Initialize first column - only one way to reach any cell in first column
        for row_index in range(rows):
            dynamic_programming_table[row_index][0] = 1
        ...
        for current_row in range(1, rows):
            for current_column in range(1, columns):
                ...
                paths_from_above = dynamic_programming_table[current_row - 1][current_column]
                paths_from_left = dynamic_programming_table[current_row][current_column - 1]
                dynamic_programming_table[current_row][current_column] = paths_from_above + paths_from_left
                
        return dynamic_programming_table[rows - 1][columns - 1]